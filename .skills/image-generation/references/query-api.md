...
|-------|-------|
| Method | `POST` |
| URL | `https://app-d9uo9b7z6l8h-api-GYX1lzGw0DQa.gateway.appmedo.com/image-generation/task` |
...
async function queryTask(taskId: string): Promise<TaskResult> {
  const response = await fetch(
    "https://app-d9uo9b7z6l8h-api-GYX1lzGw0DQa.gateway.appmedo.com/image-generation/task",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
